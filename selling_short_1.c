...
void func_8(int iParam0, int iParam1, int iParam2, int iParam3)
{
	...
	else
	{
		...
		{
			if (((!is_ped_injured(player_ped_id()) && !func_13(player_id())) && !func_10(player_id(), 0)) && !func_9())
			{
				set_entity_invincible(player_ped_id(), false);
			}
		}
		else if (!is_ped_injured(player_ped_id()) && !func_13(player_id()))
		{
			set_entity_invincible(player_ped_id(), false);
		}
		Global_69487 = 0;
...
bool func_30(char* sParam0, var uParam1, int iParam2)
{
	int iVar0;
