...
void func_5()
{
	if (Local_192.f_5 == 0)
	{
		if (!GAMEPLAY::IS_BIT_SET(Local_192.f_1, 6))
		{
			...
			{
				GAMEPLAY::SET_BIT(&(Local_192.f_1), 6);
			}
		}
	}
}
...
void func_10()
{
	...
	{
		GAMEPLAY::SET_BIT(&(Local_192.f_1), 3);
	}
}
	...
	if (network_is_participant_active(int_to_participantindex(Local_192.f_33)))
	{
		if (Local_252[Local_192.f_33 /*6*/].f_4 > 0)
		{
			...
			if (!GAMEPLAY::IS_BIT_SET(Local_192.f_4, iVar0))
			{
				...
				if (func_13())
				{
					GAMEPLAY::SET_BIT(&(Local_192.f_4), iVar0);
				}
			}
		}
	}
	...
	if (iParam4 && !iParam7)
	{
		if (func_705(player_id(), 1, 1))
		{
			if (!is_screen_faded_out())
			{
				...
				if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(func_64(player_id()), vParam0, 1) <= fVar2 + fParam3)
				{
					if (CAM::IS_SPHERE_VISIBLE(vParam0, fParam3))
					{
						return true;
				...
				if (iParam4 == 1 || (iParam4 == 0 && iVar1 != player_id()))
				{
					if ((func_65(iVar1) || !iParam10) && !Global_2418472[iVar1 /*313*/].f_255)
					{
						...
						if (!iParam7)
						{
							if ((iParam5 || (iParam5 == 0 && get_player_team(iVar1) != get_player_team(player_id()))) || get_player_team(iVar1) == -1)
							{
								if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(func_64(iVar1), vParam0, 1) <= fVar2 + fParam3)
								{
									if (CAM::IS_SPHERE_VISIBLE_to_player(iVar1, vParam0, fParam3))
									{
										return true;
				...
				if (_0x9DE986FC9A87C474(player_id(), iVar1))
				{
					if (!iParam7 || (!PED::IS_PED_INJURED(get_player_ped(iVar1)) && func_65(iVar1)))
					{
						if ((!iParam6 || (iParam6 == 1 && get_player_team(player_id()) != get_player_team(iVar1))) || get_player_team(player_id()) == -1)
						{
							...
							{
								if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(func_64(iVar1), vParam0, 1) < fParam3)
								{
									return true;
								}
							}
...
void func_154()
{
	...
	if (Local_192 != 4)
	{
		...
		while (iLocal_447 < _network_get_num_participants_host())
		{
			if (network_is_participant_active(int_to_participantindex(iLocal_447)))
			{
				...
				if (!GAMEPLAY::IS_BIT_SET(Local_192.f_1, 4))
				{
					...
					{
						GAMEPLAY::SET_BIT(&(Local_192.f_1), 4);
			...
			else if (!GAMEPLAY::IS_BIT_SET(Local_192.f_1, 5))
			{
				if (Local_192.f_33 > -1)
				{
					...
					{
						GAMEPLAY::SET_BIT(&(Local_192.f_1), 5);
					}
				}
			}
			iLocal_447++;
		...
		if (!GAMEPLAY::IS_BIT_SET(Local_192.f_1, true))
		{
			...
			if (iVar0 >= func_155())
			{
				GAMEPLAY::SET_BIT(&(Local_192.f_1), true);
			}
		...
		if (!GAMEPLAY::IS_BIT_SET(Local_192.f_1, 2))
		{
			if (!iVar1)
			{
				GAMEPLAY::SET_BIT(&(Local_192.f_1), 2);
			}
		}
	}
}
...
bool func_156()
{
	...
	if (func_160() != -1)
	{
		...
		while (iVar0 < 3)
		{
			...
			{
				iVar1 = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(false, 20);
				if (func_157(iVar1))
				{
...
bool func_157(int iParam0)
{
	...
	while (iVar0 < 3)
	{
		if (iVar0 < iParam0)
		{
			if (!func_159(Local_192.f_38[iVar0 /*3*/], 0f, 0f, 0f, 0))
			{
				fVar1 = INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(Local_192.f_38[iVar0 /*3*/], vLocal_753[iParam0 /*3*/], 1);
				if (fVar1 < func_158())
				{
					return false;
				}
		...
		if (GAMEPLAY::IS_BIT_SET(Local_192.f_1, true) || GAMEPLAY::IS_BIT_SET(Local_192.f_1, 4))
		{
			if (GAMEPLAY::IS_BIT_SET(iLocal_446, 4))
			{
				if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 6))
				{
					...
					{
						GAMEPLAY::SET_BIT(&iLocal_446, 6);
						GAMEPLAY::SET_BIT(&iLocal_446, 2);
		...
		if (GAMEPLAY::IS_BIT_SET(iLocal_446, 9))
		{
			if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 10))
			{
				...
				if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 15))
				{
					...
					{
						GAMEPLAY::SET_BIT(&iLocal_446, 15);
					}
				}
				if (GAMEPLAY::IS_BIT_SET(iLocal_446, 15))
				{
					if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 13))
					{
						...
						{
							GAMEPLAY::SET_BIT(&iLocal_446, 13);
						}
					}
					if (GAMEPLAY::IS_BIT_SET(iLocal_446, 13))
					{
						GAMEPLAY::SET_BIT(&iLocal_446, 10);
					}
				}
			}
		}
...
void func_190(float fParam0)
{
	...
	if (GAMEPLAY::GET_HASH_KEY(get_this_script_name()) == func_191())
	{
		return;
	}
												...
												if (ENTITY::DOES_ENTITY_EXIST(Var0.f_1))
												{
													if (is_entity_a_ped(Var0.f_1))
													{
														...
														if (is_ped_a_player(iVar10))
														{
															...
															if (iVar13 == player_id())
															{
																GAMEPLAY::SET_BIT(&(Local_252[participant_id_to_int() /*6*/].f_1), 5);
															}
														}
													}
												}
...
void func_343()
{
	...
	if (!GAMEPLAY::IS_BIT_SET(Local_252[participant_id_to_int() /*6*/].f_1, 4))
	{
		...
		if (GAMEPLAY::IS_BIT_SET(iLocal_445, 9))
		{
			...
			{
				GAMEPLAY::SET_BIT(&(Local_252[participant_id_to_int() /*6*/].f_1), 4);
			}
		}
	}
}
...
int func_511(int iParam0)
{
	...
	if (func_518(iParam0))
	{
		...
		if (iVar1 < func_512(iParam0))
		{
			...
			{
				iVar0 += func_516(GAMEPLAY::GET_RANDOM_INT_IN_RANGE(true, 6));
				iVar2++;
			}
		}
	}
		...
		if (iParam0 <= 30000)
		{
			if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 11))
			{
				if (prepare_music_event("APT_COUNTDOWN_30S_KILL"))
				{
					...
					GAMEPLAY::SET_BIT(&iLocal_446, 11);
				}
			}
			if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 14))
			{
				...
				{
					GAMEPLAY::SET_BIT(&iLocal_446, 14);
				}
			}
			if (GAMEPLAY::IS_BIT_SET(iLocal_446, 14))
			{
				if (iParam0 <= 27000)
				{
					if (!GAMEPLAY::IS_BIT_SET(iLocal_446, 12))
					{
						set_user_radio_control_enabled(1);
						set_audio_flag("AllowScoreAndRadio", 0);
						GAMEPLAY::SET_BIT(&iLocal_446, 12);
					}
					if (iParam0 <= 500)
					{
						if (trigger_music_event("APT_FADE_IN_RADIO"))
...
void func_554()
{
	...
	if (!GAMEPLAY::IS_BIT_SET(Global_2563397, 20))
	{
		if (!GAMEPLAY::IS_BIT_SET(Global_2563397, 21))
		{
			if (iLocal_747 < 0)
			{
				GAMEPLAY::SET_BIT(&Global_2563397, 20);
			}
		}
	}
	if (iLocal_748 == -1)
	{
		iLocal_748 = GAMEPLAY::GET_RANDOM_INT_IN_RANGE(false, 3);
	}
	if (iLocal_747 < 0)
	{
		if (GAMEPLAY::IS_BIT_SET(Global_2563397, 21))
...
void func_555()
{
	...
	switch (iLocal_747)
	{
		...
		case 1:
			...
			{
				GAMEPLAY::SET_BIT(&Local_458, false);
				GAMEPLAY::SET_BIT(&(Local_458.f_1), false);
				func_363();
				iLocal_749 = 10;
				if (iLocal_748 == 2)
				{
		...
		case 2:
			...
			else if (iVar0)
			{
				...
				GAMEPLAY::SET_BIT(&(Local_252[participant_id_to_int() /*6*/].f_1), 3);
				iLocal_748++;
				if (iLocal_748 >= 3)
				{
					iLocal_748 = 0;
...
void func_570(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4, auto uParam5, auto uParam6, int iParam7, int iParam8, int iParam9, int iParam10, int iParam11, int iParam12, int iParam13, int iParam14, int iParam15, int iParam16, int iParam17)
{
	...
	if (func_595(iParam16))
	{
		...
		if (iLocal_76 > 4)
		{
			...
			if (((is_control_just_pressed(2, 195) || is_control_just_pressed(2, 196)) || CONTROLS::IS_CONTROL_PRESSED(2, 195)) || CONTROLS::IS_CONTROL_PRESSED(2, 196))
			{
			}
			if (_is_input_disabled(2))
			{
...
void func_644()
{
	if (!GAMEPLAY::IS_BIT_SET(Local_252[participant_id_to_int() /*6*/].f_1, true))
	{
		if (!func_662())
		{
			...
			{
				func_188(1);
				GAMEPLAY::SET_BIT(&(Local_252[participant_id_to_int() /*6*/].f_1), true);
			}
		}
	}
}
