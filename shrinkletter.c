#region Local Var
	auto uLocal_0 = 0;
	auto uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	auto uLocal_12 = 0;
	auto uLocal_13 = 0;
	float fLocal_14 = 0;
	auto uLocal_15 = 0;
	auto uLocal_16 = 0;
	int iLocal_17 = 0;
	auto uLocal_18 = 0;
	auto uLocal_19 = 0;
	char* sLocal_20 = 0;
	float fLocal_21 = 0;
	auto uLocal_22 = 0;
	auto uLocal_23 = 0;
	auto uLocal_24 = 0;
	float fLocal_25 = 0;
	float fLocal_26 = 0;
	auto uLocal_27 = 0;
	int iLocal_28 = 0;
	auto uLocal_29 = 0;
	auto uLocal_30 = 0;
	float fLocal_31 = 0;
	float fLocal_32 = 0;
	float fLocal_33 = 0;
	auto uLocal_34 = 0;
	auto uLocal_35 = 0;
	auto uLocal_36 = 8;
	auto uLocal_37 = 0;
	auto uLocal_38 = 0;
	auto uLocal_39 = 0;
	auto uLocal_40 = 0;
	auto uLocal_41 = 0;
	auto uLocal_42 = 0;
	auto uLocal_43 = 0;
	auto uLocal_44 = 0;
	auto uLocal_45 = 2;
	auto uLocal_46 = 0;
	auto uLocal_47 = 0;
	auto uLocal_48 = 8;
	auto uLocal_49 = 0;
	auto uLocal_50 = 0;
	auto uLocal_51 = 0;
	auto uLocal_52 = 0;
	auto uLocal_53 = 0;
	auto uLocal_54 = 0;
	auto uLocal_55 = 0;
	auto uLocal_56 = 0;
	auto uLocal_57 = 8;
	auto uLocal_58 = 0;
	auto uLocal_59 = 0;
	auto uLocal_60 = 0;
	auto uLocal_61 = 0;
	auto uLocal_62 = 0;
	auto uLocal_63 = 0;
	auto uLocal_64 = 0;
	auto uLocal_65 = 0;
	auto uLocal_66 = 0;
	auto uLocal_67 = 0;
	auto uLocal_68 = 0;
	int iLocal_69 = 0;
	int iLocal_70 = 0;
	int iLocal_71 = 0;
	int iLocal_72 = 0;
	auto uLocal_73 = 0;
	auto uLocal_74 = 0;
	auto uLocal_75 = 0;
	auto uLocal_76 = 0;
	auto uLocal_77 = 0;
	auto uLocal_78 = 0;
	auto uLocal_79 = 0;
	auto uLocal_80 = 0;
	auto uLocal_81 = 0;
	int iLocal_82 = 0;
	int iLocal_83 = 0;
	auto uLocal_84 = 0;
	auto uLocal_85 = 0;
	auto uLocal_86 = 0;
	auto uLocal_87 = 0;
	auto uLocal_88 = 0;
	auto uLocal_89 = 0;
	auto uLocal_90 = 0;
	auto uLocal_91 = 0;
	auto uLocal_92 = 0;
	auto uLocal_93 = 0;
	auto uLocal_94 = 0;
	auto uLocal_95 = 0;
	auto uLocal_96 = 0;
	auto uLocal_97 = 0;
	auto uLocal_98 = 0;
	auto uLocal_99 = 0;
	auto uLocal_100 = 0;
	auto uLocal_101 = 0;
	int iLocal_102 = 0;
	auto uLocal_103 = 0;
	auto uLocal_104 = 8;
	auto uLocal_105 = 0;
	auto uLocal_106 = 0;
	auto uLocal_107 = 0;
	auto uLocal_108 = 4;
	auto uLocal_109 = 0;
	auto uLocal_110 = 0;
	auto uLocal_111 = 0;
	auto uLocal_112 = 0;
	auto uLocal_113 = 0;
	auto uLocal_114 = 0;
	auto uLocal_115 = 0;
	auto uLocal_116 = 0;
	auto uLocal_117 = 0;
	auto uLocal_118 = 0;
	auto uLocal_119 = 0;
	auto uLocal_120 = 0;
	auto uLocal_121 = 0;
	auto uLocal_122 = 0;
	auto uLocal_123 = 4;
	auto uLocal_124 = 0;
	auto uLocal_125 = 0;
	auto uLocal_126 = 0;
	auto uLocal_127 = 0;
	auto uLocal_128 = 0;
	auto uLocal_129 = 0;
	auto uLocal_130 = 0;
	auto uLocal_131 = 0;
	auto uLocal_132 = 0;
	auto uLocal_133 = 0;
	auto uLocal_134 = 0;
	auto uLocal_135 = 0;
	auto uLocal_136 = 0;
	auto uLocal_137 = 0;
	auto uLocal_138 = 4;
	auto uLocal_139 = 0;
	auto uLocal_140 = 0;
	auto uLocal_141 = 0;
	auto uLocal_142 = 0;
	auto uLocal_143 = 0;
	auto uLocal_144 = 0;
	auto uLocal_145 = 0;
	auto uLocal_146 = 0;
	auto uLocal_147 = 0;
	auto uLocal_148 = 0;
	auto uLocal_149 = 0;
	auto uLocal_150 = 0;
	auto uLocal_151 = 0;
	auto uLocal_152 = 0;
	auto uLocal_153 = 4;
	auto uLocal_154 = 0;
	auto uLocal_155 = 0;
	auto uLocal_156 = 0;
	auto uLocal_157 = 0;
	auto uLocal_158 = 0;
	auto uLocal_159 = 0;
	auto uLocal_160 = 0;
	auto uLocal_161 = 0;
	auto uLocal_162 = 0;
	auto uLocal_163 = 0;
	auto uLocal_164 = 0;
	auto uLocal_165 = 0;
	auto uLocal_166 = 0;
	auto uLocal_167 = 0;
	auto uLocal_168 = 4;
	auto uLocal_169 = 0;
	auto uLocal_170 = 0;
	auto uLocal_171 = 0;
	auto uLocal_172 = 0;
	auto uLocal_173 = 0;
	auto uLocal_174 = 0;
	auto uLocal_175 = 0;
	auto uLocal_176 = 0;
	auto uLocal_177 = 0;
	auto uLocal_178 = 0;
	auto uLocal_179 = 0;
	auto uLocal_180 = 0;
	auto uLocal_181 = 0;
	auto uLocal_182 = 0;
	auto uLocal_183 = 4;
	auto uLocal_184 = 0;
	auto uLocal_185 = 0;
	auto uLocal_186 = 0;
	auto uLocal_187 = 0;
	auto uLocal_188 = 0;
	auto uLocal_189 = 0;
	auto uLocal_190 = 0;
	auto uLocal_191 = 0;
	auto uLocal_192 = 0;
	auto uLocal_193 = 0;
	auto uLocal_194 = 0;
	auto uLocal_195 = 0;
	auto uLocal_196 = 0;
	auto uLocal_197 = 0;
	auto uLocal_198 = 4;
	auto uLocal_199 = 0;
	auto uLocal_200 = 0;
	auto uLocal_201 = 0;
	auto uLocal_202 = 0;
	auto uLocal_203 = 0;
	auto uLocal_204 = 0;
	auto uLocal_205 = 0;
	auto uLocal_206 = 0;
	auto uLocal_207 = 0;
	auto uLocal_208 = 0;
	auto uLocal_209 = 0;
	auto uLocal_210 = 0;
	auto uLocal_211 = 0;
	auto uLocal_212 = 0;
	auto uLocal_213 = 4;
	auto uLocal_214 = 0;
	auto uLocal_215 = 0;
	auto uLocal_216 = 0;
	auto uLocal_217 = 0;
	auto uLocal_218 = 0;
	auto uLocal_219 = 0;
	auto uLocal_220 = 0;
	auto uLocal_221 = 0;
	auto uLocal_222 = 0;
	auto uLocal_223 = 0;
	auto uLocal_224 = 0;
	auto uLocal_225 = 0;
#endregion

void main()
{
	struct<4> Var0;
	
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_14 = 0.001f;
	iLocal_17 = -1;
	sLocal_20 = "NULL";
	fLocal_21 = 0f;
	fLocal_25 = -0.0375f;
	fLocal_26 = 0.17f;
	iLocal_28 = 3;
	fLocal_31 = 80f;
	fLocal_32 = 140f;
	fLocal_33 = 180f;
	iLocal_69 = 1;
	iLocal_70 = 65;
	iLocal_71 = 49;
	iLocal_72 = 64;
	if (has_force_cleanup_occurred(18))
	{
		func_58();
	}
	request_additional_text("REPORT", false);
	while (!has_additional_text_loaded(false))
	{
		wait(false);
	}
	while (true)
	{
		switch (iLocal_82)
		{
			case 0:
				break;
			
			case 1:
				func_49(1, 1, 1, 0);
				set_player_control(player_id(), false, 0);
				display_radar(false);
				display_hud(false);
				iLocal_83 = unk_0x67D02A194A2FC2BD("PSYCHOLOGY_REPORT");
				while (!has_scaleform_movie_loaded(iLocal_83))
				{
					func_43();
					wait(false);
				}
				request_additional_text("REPORT", 3);
				while (!has_additional_text_loaded(3))
				{
					func_43();
					wait(false);
				}
				_push_scaleform_movie_function(iLocal_83, "SET_PLAYER_NAME");
				_begin_text_component("STRING");
				_add_text_component_item_string("PATIENT");
				_end_text_component();
				_begin_text_component("GAMERTAG");
				if (network_is_signed_in())
				{
					add_text_component_substring_player_name(get_player_name(player_id()));
				}
				else
				{
					_add_text_component_item_string("ACCNA_MIKE");
				}
				_end_text_component();
				_pop_scaleform_movie_function_void();
				_push_scaleform_movie_function(iLocal_83, "SET_LETTER_TEXT");
				func_42("HEADER_1");
				func_42("HEADER_2");
				Var0 = {func_41(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_40(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_39(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_38(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_37(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_35(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_33(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_32(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_31(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_30(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_28(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_24(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_23(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_21(&uLocal_84)};
				func_42(&Var0);
				Var0 = {func_20(&uLocal_84)};
				func_42(&Var0);
				_pop_scaleform_movie_function_void();
				while (!func_13(&uLocal_100, &uLocal_101, &uLocal_84))
				{
					func_43();
					wait(false);
				}
				func_12(&iLocal_102);
				func_11(&iLocal_102, 0, 0, 0, 1);
				func_10(&iLocal_102, "CONTINUE", 2, 201, 1, 1, 0);
				func_9(&iLocal_102, 1);
				func_8(&iLocal_102, 1);
				if (is_screen_faded_out())
				{
					if (!is_screen_fading_in())
					{
						func_43();
						do_screen_fade_in(800);
					}
					while (!is_screen_faded_in())
					{
						func_43();
						_set_2d_layer(1);
						draw_scaleform_movie_fullscreen(iLocal_83, 255, 255, 255, 255, 0);
						wait(false);
					}
				}
				func_43();
				_set_2d_layer(1);
				draw_scaleform_movie_fullscreen(iLocal_83, 255, 255, 255, 255, 0);
				iLocal_82 = 2;
				break;
			
			case 2:
				_set_2d_layer(1);
				draw_scaleform_movie_fullscreen(iLocal_83, 255, 255, 255, 255, 0);
				func_1(&iLocal_102, 1128792064, 1, 0, 1, 1065353216);
				if (is_control_pressed(2, 201))
				{
					iLocal_82 = 3;
				}
				break;
			
			case 3:
				func_12(&iLocal_102);
				do_screen_fade_out(3000);
				while (is_screen_fading_out())
				{
					draw_scaleform_movie_fullscreen(iLocal_83, 255, 255, 255, 255, 0);
					func_43();
					wait(false);
				}
				set_scaleform_movie_as_no_longer_needed(&iLocal_83);
				func_49(0, 1, 1, 0);
				set_player_control(player_id(), true, 0);
				display_radar(true);
				display_hud(true);
				func_58();
				break;
		}
		if (iLocal_82 != 0)
		{
			func_43();
		}
		wait(false);
	}
}

void func_1(int iParam0, Vector3 fParam1, int iParam2, int iParam3, int iParam4, Vector3 fParam5)
{
	int iVar0;
	int iVar1;
	int iVar2;
	char* sVar3;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	Vector3 fVar8;
	
	if (((is_screen_fading_out() || is_screen_fading_in()) || is_screen_faded_out()) || is_frontend_fading())
	{
		if (!iParam3)
		{
			return;
		}
	}
	if (!func_7(iParam0))
	{
		return;
	}
	hide_loading_on_fade_this_frame();
	_set_2d_layer(iParam2);
	if (!func_6(*iParam0.f_1, 256) || (func_6(*iParam0.f_1, 8192) && _0x6CD79468A1E595C6(2)))
	{
		_push_scaleform_movie_function(*iParam0, "SET_CLEAR_SPACE");
		_push_scaleform_movie_function_parameter_float(fParam1);
		_pop_scaleform_movie_function_void();
		_push_scaleform_movie_function(*iParam0, "SET_MAX_WIDTH");
		_push_scaleform_movie_function_parameter_float(fParam5);
		_pop_scaleform_movie_function_void();
		_push_scaleform_movie_function(*iParam0, "SET_DATA_SLOT_EMPTY");
		_pop_scaleform_movie_function_void();
		if (is_pc_version())
		{
			_push_scaleform_movie_function(*iParam0, "TOGGLE_MOUSE_BUTTONS");
			_push_scaleform_movie_function_parameter_bool(func_6(*iParam0.f_1, 4096));
			_pop_scaleform_movie_function_void();
		}
		iVar5 = false;
		iVar6 = 0;
		while (iVar6 < *iParam0.f_123)
		{
			switch (*(iParam0.f_2[iVar6 /*15*/]).f_2)
			{
				case 0:
					iVar4 = true;
					break;
				
				case 1:
					iVar4 = _is_input_disabled(2);
					break;
				
				case 2:
					iVar4 = !_is_input_disabled(2);
					break;
				
				default:
					iVar4 = false;
					break;
			}
			if (iVar4)
			{
				if (_push_scaleform_movie_function(*iParam0, "SET_DATA_SLOT"))
				{
					_push_scaleform_movie_function_parameter_int(iVar5);
					iVar5++;
					iVar7 = false;
					while (iVar7 < *(iParam0.f_2[iVar6 /*15*/]).f_14)
					{
						iVar0 = *(iParam0.f_2[iVar6 /*15*/].f_3)[iVar7 /*2*/];
						iVar1 = *(iParam0.f_2[iVar6 /*15*/].f_3[iVar7 /*2*/]).f_1;
						iVar2 = is_bit_set(*(iParam0.f_2[iVar6 /*15*/]).f_13, iVar7);
						if (!is_bit_set(*(iParam0.f_2[iVar6 /*15*/]).f_12, iVar7))
						{
							sVar3 = _get_control_action_name(iVar0, iVar1, iVar2);
						}
						else
						{
							sVar3 = _0x80C2FD58D720C801(iVar0, iVar1, iVar2);
						}
						if (!is_string_null_or_empty(sVar3))
						{
							func_5(sVar3);
						}
						iVar7++;
					}
					if (!is_string_null_or_empty(*iParam0.f_2[iVar6 /*15*/]))
					{
						func_42(*iParam0.f_2[iVar6 /*15*/]);
					}
					if (is_pc_version())
					{
						if (func_6(*iParam0.f_1, 4096))
						{
							if (*(iParam0.f_2[iVar6 /*15*/]).f_1)
							{
								_push_scaleform_movie_function_parameter_bool(true);
								_push_scaleform_movie_function_parameter_int(*(iParam0.f_2[iVar6 /*15*/].f_3[0 /*2*/]).f_1);
							}
							else
							{
								_push_scaleform_movie_function_parameter_bool(false);
								_push_scaleform_movie_function_parameter_int(-1);
							}
						}
					}
					_pop_scaleform_movie_function_void();
				}
			}
			iVar6++;
		}
		fVar8 = func_4(iParam4, func_4(func_6(*iParam0.f_1, 32), 1f, 0f), -1f);
		_push_scaleform_movie_function(*iParam0, "DRAW_INSTRUCTIONAL_BUTTONS");
		_push_scaleform_movie_function_parameter_float(fVar8);
		_pop_scaleform_movie_function_void();
		_push_scaleform_movie_function(*iParam0, "SET_BACKGROUND_COLOUR");
		_push_scaleform_movie_function_parameter_float(0f);
		_push_scaleform_movie_function_parameter_float(0f);
		_push_scaleform_movie_function_parameter_float(0f);
		_push_scaleform_movie_function_parameter_float(80f);
		_pop_scaleform_movie_function_void();
		func_3(iParam0.f_1, 256);
		func_2(iParam0.f_1, 128);
	}
	draw_scaleform_movie_fullscreen(*iParam0, 255, 255, 255, false, 0);
}

void func_2(auto uParam0, int iParam1)
{
	*uParam0 -= (*uParam0 && iParam1);
}

void func_3(auto uParam0, int iParam1)
{
	*uParam0 = (*uParam0 || iParam1);
}

float func_4(int iParam0, float fParam1, float fParam2)
{
	if (iParam0)
	{
		return fParam1;
	}
	return fParam2;
}

void func_5(char* sParam0)
{
	_0xE83A3E3557A56640(sParam0);
}

int func_6(auto uParam0, int iParam1)
{
	return (uParam0 && iParam1) != 0;
}

bool func_7(auto uParam0)
{
	if (*uParam0 != 0)
	{
		if (has_scaleform_movie_loaded(*uParam0))
		{
			func_3(uParam0.f_1, 1);
			return true;
		}
	}
	return false;
}

void func_8(int iParam0, int iParam1)
{
	if (iParam1)
	{
		func_3(iParam0.f_1, 16);
	}
	else
	{
		func_2(iParam0.f_1, 16);
	}
}

void func_9(int iParam0, int iParam1)
{
	if (iParam1)
	{
		func_3(iParam0.f_1, 1024);
	}
	else
	{
		func_2(iParam0.f_1, 1024);
	}
}

bool func_10(int iParam0, char* sParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6)
{
	int iVar0;
	int iVar1;
	
	if (*iParam0 == 0)
	{
		return false;
	}
	iVar0 = 0;
	if (iParam5 == 1)
	{
		iVar0 = 1;
	}
	iVar1 = *iParam0.f_123;
	if (iVar1 < 8)
	{
		*iParam0.f_2[iVar1 /*15*/] = sParam1;
		*(iParam0.f_2[iVar1 /*15*/]).f_1 = iVar0;
		*(iParam0.f_2[iVar1 /*15*/]).f_2 = iParam6;
		*(iParam0.f_2[iVar1 /*15*/]).f_12 = 0;
		*(iParam0.f_2[iVar1 /*15*/]).f_13 = 0;
		*(iParam0.f_2[iVar1 /*15*/]).f_14 = 0;
		*(iParam0.f_2[iVar1 /*15*/].f_3)[0 /*2*/] = iParam2;
		*(iParam0.f_2[iVar1 /*15*/].f_3[0 /*2*/]).f_1 = iParam3;
		if (iParam4 == 1)
		{
			GAMEPLAY::GAMEPLAY::SET_BIT(iParam0.f_2[iVar1 /*15*/].f_13, false);
		}
		*(iParam0.f_2[iVar1 /*15*/]).f_14++;
		*iParam0.f_123++;
		return true;
	}
	return false;
}

void func_11(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4)
{
	if (*iParam0 == 0)
	{
		*iParam0 = request_scaleform_movie_instance("instructional_buttons");
	}
	*iParam0.f_1 = 0;
	*iParam0.f_123 = 0;
	if (iParam1)
	{
		func_3(iParam0.f_1, 32);
	}
	if (has_scaleform_movie_loaded(*iParam0))
	{
		func_3(iParam0.f_1, 1);
		if (iParam2)
		{
			set_scaleform_movie_to_use_system_time(*iParam0, 1);
		}
	}
	if (is_pc_version())
	{
		if (iParam3)
		{
			func_3(iParam0.f_1, 4096);
		}
	}
	if (iParam4)
	{
		func_3(iParam0.f_1, 8192);
	}
}

void func_12(int iParam0)
{
	if (*iParam0 != 0)
	{
		set_scaleform_movie_as_no_longer_needed(iParam0);
		*iParam0 = 0;
	}
	*iParam0.f_1 = 0;
	*iParam0.f_123 = 0;
}

bool func_13(auto uParam0, auto uParam1, auto uParam2)
{
	int iVar0;
	
	if (func_17())
	{
		return true;
	}
	switch (*uParam0)
	{
		case 0:
			if (func_16())
			{
				return false;
			}
			func_15();
			*uParam0++;
			break;
		
		case 1:
			datafile_create();
			iVar0 = _get_root_object();
			_object_value_add_integer(iVar0, "in", *uParam2);
			_object_value_add_string(iVar0, "st", uParam2.f_1);
			_object_value_add_string(iVar0, "mp", uParam2.f_2);
			_object_value_add_string(iVar0, "ms", uParam2.f_3);
			_object_value_add_string(iVar0, "sc", uParam2.f_5);
			_object_value_add_string(iVar0, "pr", uParam2.f_6);
			_object_value_add_string(iVar0, "fa", uParam2.f_7);
			_object_value_add_string(iVar0, "sm", uParam2.f_8);
			_object_value_add_string(iVar0, "kp", uParam2.f_9);
			_object_value_add_string(iVar0, "sv", uParam2.f_10);
			_object_value_add_string(iVar0, "yo", uParam2.f_11);
			_object_value_add_string(iVar0, "fi", uParam2.f_12);
			_object_value_add_string(iVar0, "rc", uParam2.f_13);
			_object_value_add_string(iVar0, "co", uParam2.f_14);
			_object_value_add_integer(iVar0, "su", *uParam2.f_15);
			_0x83BCCE3224735F05("gta5/psych/index.json");
			*uParam0++;
			break;
		
		case 2:
			if (func_14(uParam1, 1))
			{
				return true;
			}
			break;
	}
	return false;
}

bool func_14(auto uParam0, int iParam1)
{
	if (!_0x4DFDD9EB705F8140(uParam0))
	{
		if (*uParam0)
		{
			if (_get_root_object() != 0 && iParam1)
			{
				datafile_delete();
			}
			return true;
		}
		else
		{
			if (_get_root_object() != 0 && iParam1)
			{
				datafile_delete();
			}
			return true;
		}
	}
	return false;
}

void func_15()
{
	if (_get_root_object() != 0)
	{
		datafile_delete();
	}
}

bool func_16()
{
	if (_0x9FEDF86898F100E9())
	{
		return true;
	}
	else if (_0xD53ACDBEF24A46E8())
	{
		return true;
	}
	else if (_0x45E816772E93A9DB())
	{
		return true;
	}
	return false;
}

bool func_17()
{
	if (!func_18())
	{
		return true;
	}
	return false;
}

bool func_18()
{
	if (func_19())
	{
		return false;
	}
	if (network_is_cloud_available() == 0)
	{
		return false;
	}
	return true;
}

auto func_19()
{
	return Global_2443548;
}

struct<4> func_20(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "SUMMARY_", 16);
	iVar4 = get_random_int_in_range(true, 24);
	*uParam0.f_15 = iVar4;
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_21(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "COLLECT_", 16);
	if (((func_22(110) || func_22(95)) || func_22(106)) || func_22(107))
	{
		StringCopy(uParam0.f_14, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
		iVar4 = get_random_int_in_range(true, 11);
	}
	else
	{
		StringCopy(uParam0.f_14, "N", 4);
		StringConCat(&cVar0, "N", 16);
		iVar4 = get_random_int_in_range(true, 12);
	}
	StringIntConCat(uParam0.f_14, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

int func_22(int iParam0)
{
	if (iParam0 == 146 || iParam0 == -1)
	{
		return 0;
	}
	return Global_101154.f_7775.f_99.f_58[iParam0];
}

struct<4> func_23(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	
	StringCopy(&cVar0, "RAND_", 16);
	stat_get_int(joaat("num_rndpeople_completed"), &iVar5, -1);
	stat_get_int(joaat("num_rndevents_completed"), &iVar6, -1);
	iVar7 = iVar5 + iVar6;
	if (iVar7 > 10)
	{
		StringCopy(uParam0.f_13, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_13, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_13, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_24(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "FIT_", 16);
	if ((((((((func_25(1, 1) > 50 && func_25(1, 2) > 50) && func_25(1, 3) > 50) && func_25(0, 1) > 50) && func_25(0, 2) > 50) && func_25(0, 3) > 50) && func_25(2, 1) > 50) && func_25(2, 2) > 50) && func_25(2, 3) > 50)
	{
		StringCopy(uParam0.f_12, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_12, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 11);
	StringIntConCat(uParam0.f_12, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

auto func_25(int iParam0, int iParam1)
{
	auto uVar0;
	int iVar1;
	
	iVar1 = func_26(iParam0, iParam1);
	stat_get_int(iVar1, &uVar0, -1);
	return uVar0;
}

auto func_26(int iParam0, int iParam1)
{
	auto uVar0;
	auto uVar1;
	
	func_27(iParam0, iParam1, &uVar0, &uVar1);
	return uVar0;
}

void func_27(int iParam0, int iParam1, auto uParam2, auto uParam3)
{
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("sp0_special_ability_unlocked");
					break;
				
				case 1:
					*uParam2 = joaat("sp0_stamina");
					break;
				
				case 3:
					*uParam2 = joaat("sp0_lung_capacity");
					break;
				
				case 2:
					*uParam2 = joaat("sp0_strength");
					break;
				
				case 4:
					*uParam2 = joaat("sp0_wheelie_ability");
					break;
				
				case 5:
					*uParam2 = joaat("sp0_flying_ability");
					break;
				
				case 6:
					*uParam2 = joaat("sp0_shooting_ability");
					break;
				
				case 7:
					*uParam2 = joaat("sp0_stealth_ability");
					break;
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("sp1_special_ability_unlocked");
					break;
				
				case 1:
					*uParam2 = joaat("sp1_stamina");
					break;
				
				case 3:
					*uParam2 = joaat("sp1_lung_capacity");
					break;
				
				case 2:
					*uParam2 = joaat("sp1_strength");
					break;
				
				case 4:
					*uParam2 = joaat("sp1_wheelie_ability");
					break;
				
				case 5:
					*uParam2 = joaat("sp1_flying_ability");
					break;
				
				case 6:
					*uParam2 = joaat("sp1_shooting_ability");
					break;
				
				case 7:
					*uParam2 = joaat("sp1_stealth_ability");
					break;
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("sp2_special_ability_unlocked");
					break;
				
				case 1:
					*uParam2 = joaat("sp2_stamina");
					break;
				
				case 3:
					*uParam2 = joaat("sp2_lung_capacity");
					break;
				
				case 2:
					*uParam2 = joaat("sp2_strength");
					break;
				
				case 4:
					*uParam2 = joaat("sp2_wheelie_ability");
					break;
				
				case 5:
					*uParam2 = joaat("sp2_flying_ability");
					break;
				
				case 6:
					*uParam2 = joaat("sp2_shooting_ability");
					break;
				
				case 7:
					*uParam2 = joaat("sp2_stealth_ability");
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					*uParam3 = 64;
					break;
				
				case 1:
					*uParam3 = 65;
					break;
				
				case 3:
					*uParam3 = 67;
					break;
				
				case 2:
					*uParam3 = 66;
					break;
				
				case 4:
					*uParam3 = 68;
					break;
				
				case 5:
					*uParam3 = 69;
					break;
				
				case 6:
					*uParam3 = 70;
					break;
				
				case 7:
					*uParam3 = 71;
					break;
			}
			break;
	}
}

struct<4> func_28(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "YOGA_", 16);
	if (func_29(300, 1))
	{
		StringCopy(uParam0.f_11, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_11, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_11, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

bool func_29(int iParam0, int iParam1)
{
	if (Global_101154.f_8884[iParam0 /*12*/].f_5 == 1)
	{
		if (iParam1 == 1)
		{
		}
		return true;
	}
	return false;
}

struct<4> func_30(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	
	StringCopy(&cVar0, "VEHS_", 16);
	stat_get_int(joaat("sp0_number_stolen_cop_vehicle"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_cars"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_bikes"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_boats"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_helis"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_quadbikes"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_number_stolen_bicycles"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_cop_vehicle"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_cars"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_bikes"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_boats"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_helis"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_quadbikes"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_number_stolen_bicycles"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_cop_vehicle"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_cars"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_bikes"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_boats"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_helis"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_quadbikes"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_number_stolen_bicycles"), &iVar5, 2);
	iVar6 += iVar5;
	if (iVar6 >= 100)
	{
		StringCopy(uParam0.f_10, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_10, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_10, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_31(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	
	StringCopy(&cVar0, "PEDS_", 16);
	stat_get_int(joaat("sp0_kills_innocents"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_kills_innocents"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_kills_innocents"), &iVar5, 2);
	iVar6 += iVar5;
	if (iVar6 >= 100)
	{
		StringCopy(uParam0.f_9, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_9, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_9, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_32(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	
	StringCopy(&cVar0, "STOCK_", 16);
	stat_get_int(joaat("time_spent_on_stockmarket"), &iVar5, 0);
	if (iVar5 >= 120000)
	{
		StringCopy(uParam0.f_8, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_8, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_8, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_33(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "FAMILY_", 16);
	if (func_34(8) && func_34(5))
	{
		StringCopy(uParam0.f_7, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_7, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_7, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

int func_34(int iParam0)
{
	if (iParam0 < 9)
	{
		return is_bit_set(Global_101154.f_16793.f_175[iParam0 /*19*/].f_18, false);
	}
	return false;
}

struct<4> func_35(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "PROS_", 16);
	if ((func_36(1) > 0 || func_36(0) > 0) || func_36(2) > 0)
	{
		StringCopy(uParam0.f_6, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
	}
	else
	{
		StringCopy(uParam0.f_6, "N", 4);
		StringConCat(&cVar0, "N", 16);
	}
	iVar4 = get_random_int_in_range(true, 13);
	StringIntConCat(uParam0.f_6, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

int func_36(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return Global_101154.f_8739.f_90[0];
			break;
		
		case 1:
			return Global_101154.f_8739.f_90[1];
			break;
		
		case 2:
			return Global_101154.f_8739.f_90[2];
			break;
	}
	return 0;
}

struct<4> func_37(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	int iVar8;
	
	StringCopy(&cVar0, "STRIP_", 16);
	stat_get_int(joaat("sp0_lap_danced_bought"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_lap_danced_bought"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_lap_danced_bought"), &iVar5, 2);
	iVar6 += iVar5;
	stat_get_int(joaat("sp0_money_spent_in_strip_clubs"), &iVar7, 0);
	iVar8 += iVar7;
	stat_get_int(joaat("sp1_money_spent_in_strip_clubs"), &iVar7, 1);
	iVar8 += iVar7;
	stat_get_int(joaat("sp2_money_spent_in_strip_clubs"), &iVar7, 2);
	iVar8 += iVar7;
	if (iVar6 >= 3 || iVar8 >= 100)
	{
		StringCopy(uParam0.f_5, "Y", 4);
		StringConCat(&cVar0, "Y", 16);
		iVar4 = get_random_int_in_range(true, 13);
	}
	else
	{
		StringCopy(uParam0.f_5, "N", 4);
		StringConCat(&cVar0, "N", 16);
		iVar4 = get_random_int_in_range(true, 12);
	}
	StringIntConCat(uParam0.f_5, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_38(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	
	StringCopy(&cVar0, "CASH_", 16);
	stat_get_int(joaat("sp0_money_total_spent"), &iVar5, 0);
	iVar6 += iVar5;
	stat_get_int(joaat("sp1_money_total_spent"), &iVar5, 1);
	iVar6 += iVar5;
	stat_get_int(joaat("sp2_money_total_spent"), &iVar5, 2);
	iVar6 += iVar5;
	if (iVar6 >= 1000000)
	{
		StringCopy(uParam0.f_3, "SP", 8);
		StringConCat(&cVar0, "SP", 16);
		iVar4 = get_random_int_in_range(true, 14);
	}
	else
	{
		StringCopy(uParam0.f_3, "SA", 8);
		StringConCat(&cVar0, "SA", 16);
		iVar4 = get_random_int_in_range(true, 13);
	}
	StringIntConCat(uParam0.f_3, iVar4, 8);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_39(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	
	StringCopy(&cVar0, "CHAR_", 16);
	iVar5 = 0;
	iVar7 = 0;
	stat_get_int(joaat("sp0_total_playing_time"), &iVar7, 0);
	stat_get_int(joaat("sp1_total_playing_time"), &iVar6, 1);
	if (iVar6 > iVar7)
	{
		iVar7 = iVar6;
		iVar5 = 1;
	}
	stat_get_int(joaat("sp2_total_playing_time"), &iVar6, 2);
	if (iVar6 > iVar7)
	{
		iVar7 = iVar6;
		iVar5 = 2;
	}
	if (iVar5 == 0)
	{
		StringCopy(uParam0.f_2, "M", 4);
		StringConCat(&cVar0, "M", 16);
		iVar4 = get_random_int_in_range(true, 13);
	}
	else if (iVar5 == 1)
	{
		StringCopy(uParam0.f_2, "F", 4);
		StringConCat(&cVar0, "F", 16);
		iVar4 = get_random_int_in_range(true, 13);
	}
	else
	{
		StringCopy(uParam0.f_2, "T", 4);
		StringConCat(&cVar0, "T", 16);
		iVar4 = get_random_int_in_range(true, 12);
	}
	StringIntConCat(uParam0.f_2, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_40(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "STORY_", 16);
	if (func_22(135))
	{
		StringCopy(uParam0.f_1, "M", 4);
		StringConCat(&cVar0, "M", 16);
		iVar4 = get_random_int_in_range(true, 11);
	}
	else if (func_22(136))
	{
		StringCopy(uParam0.f_1, "T", 4);
		StringConCat(&cVar0, "T", 16);
		iVar4 = get_random_int_in_range(true, 10);
	}
	else
	{
		StringCopy(uParam0.f_1, "B", 4);
		StringConCat(&cVar0, "B", 16);
		iVar4 = get_random_int_in_range(true, 11);
	}
	StringIntConCat(uParam0.f_1, iVar4, 4);
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

struct<4> func_41(auto uParam0)
{
	char[16] cVar0;
	int iVar4;
	
	StringCopy(&cVar0, "INTRO_", 16);
	iVar4 = get_random_int_in_range(true, 21);
	*uParam0 = iVar4;
	StringIntConCat(&cVar0, iVar4, 16);
	return cVar0;
}

void func_42(char* sParam0)
{
	_begin_text_component(sParam0);
	_end_text_component();
}

void func_43()
{
	disable_control_action(2, 199, 1);
	hide_hud_component_this_frame(7);
	hide_hud_component_this_frame(7);
	hide_hud_component_this_frame(6);
	hide_hud_component_this_frame(8);
	hide_hud_component_this_frame(9);
	func_44();
}

void func_44()
{
	if (!Global_14413.f_1 == 1)
	{
		if (func_48(0))
		{
			func_45(0);
		}
		GAMEPLAY::GAMEPLAY::SET_BIT(&Global_2284, 2);
	}
}

void func_45(int iParam0)
{
	if (Global_14571)
	{
		func_47(0, 0);
	}
	if (Global_14413.f_1 == 10 || Global_14413.f_1 == 9)
	{
		GAMEPLAY::GAMEPLAY::SET_BIT(&Global_2284, 16);
	}
	if (is_mobile_phone_call_ongoing())
	{
		stop_scripted_conversation(false);
	}
	Global_15712 = 5;
	if (iParam0 == 1)
	{
		GAMEPLAY::GAMEPLAY::SET_BIT(&Global_2283, 30);
	}
	else
	{
		clear_bit(&Global_2283, 30);
	}
	if (!func_46())
	{
		Global_14413.f_1 = 3;
	}
}

bool func_46()
{
	if (Global_14413.f_1 == 1 || Global_14413.f_1 == 0)
	{
		return true;
	}
	return false;
}

void func_47(int iParam0, int iParam1)
{
	if (iParam0)
	{
		if (func_48(0))
		{
			Global_14571 = 1;
			if (iParam1)
			{
				get_mobile_phone_position(&Global_14350);
			}
			Global_14341 = {Global_14359[Global_14358 /*3*/]};
			set_mobile_phone_position(Global_14341);
		}
	}
	else if (Global_14571 == 1)
	{
		Global_14571 = 0;
		Global_14341 = {Global_14366[Global_14358 /*3*/]};
		if (iParam1)
		{
			set_mobile_phone_position(Global_14350);
		}
		else
		{
			set_mobile_phone_position(Global_14341);
		}
	}
}

bool func_48(int iParam0)
{
	if (iParam0 == 1)
	{
		if (Global_14413.f_1 > 3)
		{
			if (is_bit_set(Global_2283, 14))
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		else
		{
			return false;
		}
	}
	if (_get_number_of_instances_of_streamed_script(joaat("cellphone_flashhand")) > 0)
	{
		return true;
	}
	if (Global_14413.f_1 > 3)
	{
		return true;
	}
	return false;
}

void func_49(int iParam0, int iParam1, int iParam2, int iParam3)
{
	if (iParam0)
	{
		special_ability_deactivate_fast(player_id());
		set_all_random_peds_flee(player_id(), 1);
		set_police_ignore_player(player_id(), 1);
		func_57(1);
		_0xA8FDB297A8D25FBA();
		_0xFDB423997FA30340();
		if (Global_14413.f_1 > 3)
		{
			if (is_mobile_phone_call_ongoing())
			{
				stop_scripted_conversation(false);
			}
			if (!func_46())
			{
				Global_14413.f_1 = 3;
			}
			Global_15712 = 5;
		}
		func_56(1, iParam3, iParam2, 0);
		Global_55755 = 1;
		Global_68061 = 1;
		Global_69487 = 1;
	}
	else
	{
		func_57(0);
		_0xE1CD1E48E025E661();
		Global_55755 = 0;
		if (iParam1)
		{
			_0x03FC694AE06C5A20();
		}
		set_all_random_peds_flee(player_id(), 0);
		set_police_ignore_player(player_id(), 0);
		func_56(0, iParam3, iParam2, 0);
		if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
		{
			if (((!PED::IS_PED_INJURED(PLAYER::PLAYER_PED_ID()) && !func_54(player_id())) && !func_51(player_id(), 0)) && !func_50())
			{
				set_entity_invincible(PLAYER::PLAYER_PED_ID(), false);
			}
		}
		else if (!PED::IS_PED_INJURED(PLAYER::PLAYER_PED_ID()) && !func_54(player_id()))
		{
			set_entity_invincible(PLAYER::PLAYER_PED_ID(), false);
		}
		Global_69487 = 0;
	}
}

int func_50()
{
	return is_bit_set(Global_1587523[player_id() /*444*/].f_39.f_18, 14);
}

int func_51(int iParam0, int iParam1)
{
	int iVar0;
	
	if (iParam0 == player_id())
	{
		iVar0 = func_52(-1, 0) == 8;
	}
	else
	{
		iVar0 = Global_1587523[iParam0 /*444*/].f_195 == 8;
	}
	if (iParam1 == 1)
	{
		if (network_is_player_active(iParam0))
		{
			iVar0 = get_player_team(iParam0) == 8;
		}
	}
	return iVar0;
}

int func_52(int iParam0, int iParam1)
{
	int iVar0;
	int iVar1;
	
	iVar1 = iParam0;
	if (iVar1 == -1)
	{
		iVar1 = func_53();
	}
	if (Global_1315891[iVar1] == 1)
	{
		if (iParam1)
		{
		}
		iVar0 = 8;
	}
	else
	{
		iVar0 = Global_1312741[iVar1];
		if (iParam1)
		{
		}
	}
	return iVar0;
}

auto func_53()
{
	return Global_1312747;
}

bool func_54(int iParam0)
{
	if (func_51(iParam0, 0))
	{
		return true;
	}
	if (func_55())
	{
		if (iParam0 == player_id())
		{
			return true;
		}
	}
	if (is_bit_set(Global_2418472[iParam0 /*313*/].f_194, 2))
	{
		return true;
	}
	return false;
}

int func_55()
{
	return is_bit_set(Global_2359301, 3);
}

int func_56(int iParam0, int iParam1, auto uParam2, int iParam3)
{
	int iVar0;
	
	iVar0 = 0;
	if (is_pc_version())
	{
		if (_0xA0FE76168A189DDB() != iParam0 && uParam2)
		{
			_0x20746F7B1032A3C7(iParam0, iParam1, 1, iParam3);
			iVar0 = 1;
		}
	}
	return iVar0;
}

void func_57(int iParam0)
{
	if (iParam0 == 1)
	{
		GAMEPLAY::GAMEPLAY::SET_BIT(&Global_2283, 13);
	}
	else
	{
		clear_bit(&Global_2283, 13);
	}
}

void func_58()
{
	if (is_screen_faded_out())
	{
		do_screen_fade_in(false);
	}
	terminate_this_thread();
}

