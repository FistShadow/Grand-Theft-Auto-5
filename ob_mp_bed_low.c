...
void main()
{
	...
	while (true)
	{
		...
		{
			if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(vLocal_339, ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 0), 1) < 3f)
			{
				switch (iLocal_321)
				{
					case 0:
			...
			else if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(vLocal_339, ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 0), 1) > 3f + 2f)
			{
				func_72();
			}
		}
		...
		if ((!is_ped_in_any_vehicle(PLAYER::PLAYER_PED_ID(), 0) && !is_entity_on_fire(PLAYER::PLAYER_PED_ID())) && is_player_control_on(player_id()))
		{
			if (!is_explosion_in_sphere(-1, vParam0, 2f))
			{
				if ((((is_gameplay_cam_rendering() && !is_cutscene_playing()) && !is_cinematic_cam_rendering()) && !func_37(0)) && !func_47())
				{
					...
					if (func_45(ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1), vParam0, fParam3, 0))
					{
						return true;
					}
				}
