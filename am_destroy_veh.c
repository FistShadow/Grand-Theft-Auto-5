...
int func_23(auto uParam0, auto uParam1)
{
	...
	if (!iVar0)
	{
		if (_0xA4822F1CF23F4810(&vVar5, uParam0, &uVar2, 0f, 180f, 150f, false, 1, 1))
		{
			...
			if (iVar1 >= 1)
			{
				if (INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(-1367.557f, -3220.598f, 12.9448f, vVar5, 1) >= 600f && INTERIOR::GET_INTERIOR_AT_COORDS_WITH_TYPE(750f, -3200f, 6f, vVar5, 1) >= 700f)
				{
					if (iLocal_248 <= 5)
					{
		...
		if (!GAMEPLAY::IS_BIT_SET(Local_96.f_1, true) && !GAMEPLAY::IS_BIT_SET(vLocal_145[participant_id_to_int() /*3*/].f_1, 3))
		{
			if (network_does_network_id_exist(Local_96.f_3))
			{
				if (network_has_control_of_network_id(Local_96.f_3) || (!_0xA1607996431332DF(Local_96.f_3) && network_is_host_of_this_script()))
				{
					...
					GAMEPLAY::SET_BIT(&(vLocal_145[participant_id_to_int() /*3*/].f_1), 3);
				}
			}
		}
		iVar0 = 0;
			...
			if (!GAMEPLAY::IS_BIT_SET(Local_96.f_1, 2 + iVar0) && !GAMEPLAY::IS_BIT_SET(vLocal_145[participant_id_to_int() /*3*/].f_1, 4 + iVar0))
			{
				if (network_does_network_id_exist(Local_96.f_4[iVar0]))
				{
					if (network_has_control_of_network_id(Local_96.f_4[iVar0]) || (!_0xA1607996431332DF(Local_96.f_4[iVar0]) && network_is_host_of_this_script()))
					{
						...
						GAMEPLAY::SET_BIT(&(vLocal_145[participant_id_to_int() /*3*/].f_1), 4 + iVar0);
					}
				}
			}
			iVar0++;
		...
		if (!GAMEPLAY::IS_BIT_SET(vLocal_145[participant_id_to_int() /*3*/].f_1, false))
		{
			...
			if (ENTITY::IS_ENTITY_DEAD(net_to_veh(Local_96.f_2), 0))
			{
				if (player_id() == network_get_destroyer_of_network_id(Local_96.f_2, &uVar0))
				{
					...
					GAMEPLAY::SET_BIT(&(vLocal_145[participant_id_to_int() /*3*/].f_1), false);
				}
			}
			else
			{
				if (!GAMEPLAY::IS_BIT_SET(Local_96.f_1, false))
				{
					if (!GAMEPLAY::IS_BIT_SET(vLocal_145[participant_id_to_int() /*3*/].f_1, true))
					{
						...
						{
							GAMEPLAY::SET_BIT(&(vLocal_145[participant_id_to_int() /*3*/].f_1), true);
						}
					}
		...
		if (func_52(27, -1) == 0)
		{
			if (func_205(player_id(), 1, 1))
			{
				if (func_53(PLAYER::PLAYER_PED_ID(), net_to_veh(Local_96.f_2), 0))
				{
					...
					{
						func_50(27, 1, -1);
						GAMEPLAY::SET_BIT(&iLocal_242, false);
					}
				}
