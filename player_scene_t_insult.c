...
bool func_2()
{
	...
	while (iVar0 < iLocal_233)
	{
		...
		{
			vVar5 = {get_entity_coords(iLocal_233[iVar0], 1)};
...
void func_20(int iParam0, int iParam1)
{
				...
				else
				{
					...
					else
					{
						...
						if (iLocal_252[0] == joaat("p_trev_rope_01_s") || iLocal_252[1] == joaat("p_trev_rope_01_s"))
						{
							...
							set_entity_health(iLocal_233[iParam1], get_entity_max_health(iLocal_233[iParam1]));
			...
			case 10:
				...
				if (is_entity_playing_anim(iLocal_233[iParam1], &Local_341, &(Local_423[iParam1 /*16*/]), 2))
				{
					if (iLocal_66 == 288)
					{
						if (iLocal_727 <= 0)
						{
							iLocal_727 = get_game_timer() + get_random_int_in_range(10000, 15000);
						}
				...
				switch (iLocal_242[iParam1])
				{
					...
					case 0:
						...
						task_look_at_entity(iLocal_233[iParam1], player_ped_id(), iVar17, 0, 2);
						iLocal_245[iParam1] = get_game_timer() + iVar17 + get_random_int_in_range(500, 2000);
						iLocal_242[iParam1] = 1;
						break;
					
					case 1:
						if (get_game_timer() > iLocal_245[iParam1])
						{
				...
				if (has_entity_been_damaged_by_entity(iLocal_233[iParam1], player_ped_id(), 1))
				{
				}
				iVar18 = false;
	...
	else
	{
		...
		if (does_entity_exist(iParam0))
		{
			...
			{
				if (does_entity_exist(iLocal_249[iVar29]))
				{
...
bool func_22(int iParam0)
{
	...
	while (iVar0 < iLocal_233)
	{
		...
		{
			if (iLocal_236[iVar0] < 0 && does_entity_exist(iLocal_233[iVar0]))
			{
				return true;
			}
		}
...
bool func_30()
{
	...
	if (!func_31())
	{
		if (is_synchronized_scene_running(Global_88941))
		{
			...
			{
				if (does_entity_exist(iLocal_249[iVar0]))
				{
...
bool func_32()
{
	...
	if (is_synchronized_scene_running(Global_88940))
	{
		...
		if (!is_string_null_or_empty(&Local_721))
		{
			func_21(&uLocal_67, 2, player_ped_id(), "TREVOR", 0, 1);
...
bool func_43(int iParam0, char* sParam1, char* sParam2, char* sParam3, var uParam4, var uParam5)
{
	*uParam4 = 9;
	*uParam5 = 0;
	switch (iParam0)
...
bool func_45(int iParam0, var uParam1, var uParam2, char* sParam3)
{
	int iVar0;
