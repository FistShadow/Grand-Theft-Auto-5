...
void main()
{
	...
	iLocal_43 = 64;
	iLocal_200 = 20;
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && func_65(player_id(), 0, 1))
	{
		func_58(Local_204);
	}
	else
...
bool func_2()
{
	if (Local_86.f_9 == 0)
	{
		...
		else
		{
			if (iLocal_203 == 1)
			{
				if (GAMEPLAY::IS_BIT_SET(Local_86.f_1, false))
				{
					return true;
				}
			}
...
void func_7()
{
	...
	{
		iLocal_203 = 0;
		GAMEPLAY::GAMEPLAY::SET_BIT(&(Local_86.f_1), false);
	}
	if (func_4(Local_86.f_2))
	{
		iVar0 = 1;
		...
		if (func_65(iVar1, 1, 1))
		{
			if (GAMEPLAY::IS_BIT_SET(Local_86.f_1, false))
			{
				if (iVar0 == 1)
				{
					...
					{
						GAMEPLAY::CLEAR_BIT(&(Local_86.f_1), false);
					}
				}
				else
				{
	...
	if ((!network_does_network_id_exist(Local_86.f_3) && func_29(Local_86.f_5)) && network_does_network_id_exist(Local_86.f_2))
	{
		if (func_4(Local_86.f_2))
		{
			if (func_11(&(Local_86.f_3), Local_86.f_2, 22, Local_86.f_5, -1, 1, 1, 1))
			{
				...
				if (func_10())
				{
					PED::PED::SET_PED_RELATIONSHIP_GROUP_HASH(net_to_ped(Local_86.f_3), Global_1574232);
				}
				else
				{
					PED::PED::SET_PED_RELATIONSHIP_GROUP_HASH(net_to_ped(Local_86.f_3), Global_1574235);
				}
				...
				{
					PED::SET_PED_COMBAT_ABILITY(net_to_ped(Local_86.f_3), 2);
				}
				else
				{
				...
				{
					PED::SET_PED_COMBAT_ATTRIBUTES(net_to_ped(Local_86.f_3), 17, true);
				}
				else if (iVar0 == 2)
				{
					PED::SET_PED_COMBAT_ATTRIBUTES(net_to_ped(Local_86.f_3), 13, true);
				}
				if (func_10())
				{
					PED::SET_PED_COMBAT_ATTRIBUTES(net_to_ped(Local_86.f_3), 20, true);
				}
				if (func_10())
				{
	...
	while (iVar0 < 32)
	{
		...
		if ((iParam8 == 1 && player_id() != iVar1) || iParam8 == 0)
		{
			if (func_65(iVar1, iParam4, iParam5))
			{
				...
				{
					if (!iParam7 || (!PED::IS_PED_INJURED(get_player_ped(iVar1)) && func_17(iVar1)))
					{
...
int func_30()
{
	...
	return GAMEPLAY::IS_BIT_SET(iLocal_195, true);
}
...
void func_35()
{
	...
	if (!GAMEPLAY::IS_BIT_SET(vLocal_98[participant_id_to_int() /*3*/].f_1, false))
	{
		if (func_4(Local_86.f_2) && !func_3(Local_86.f_3))
		{
			...
			if (iVar0 < 200)
			{
				...
				GAMEPLAY::GAMEPLAY::SET_BIT(&(vLocal_98[participant_id_to_int() /*3*/].f_1), false);
			}
			else if (fVar1 < 3f)
			{
...
void func_36()
{
	if (!GAMEPLAY::IS_BIT_SET(iLocal_195, false))
	{
		if (func_4(Local_86.f_2))
		{
			...
			GAMEPLAY::GAMEPLAY::SET_BIT(&iLocal_195, false);
		}
	}
}
